...
Fixpoint eqTyp a b :=
  match a,b with
  | KRTypModuleElt,KRTypModuleElt => true
  | KRTypRegInitT,KRTypRegInitT => true
  | KRTypAttributeActionVoid,KRTypAttributeActionVoid => true
  | KRList l1,KRList l2 => eqTyp l1 l2
  | _,_ => false
  end.

(*Instance eqTypInst : Coq.Structures.Equalities.Eq KRTyp :=
  {
    eqb:= eqTyp;
  }.*)
...
refine
  ...
  (*| KRApp KRModuleElt a b => match typ,@KRRealizeExp (KRList t) a,@KRRealizeExp (KRList t) b with
                    | (KRList KRTypModuleElt),Some f,Some r => _
                    | _,_,_ => None
                    end*)
...
Goal forall e a b, (Registers a)++b=e.
  intros.
  match goal with
  | |- ?A = ?B => 
  ...
  cbv [KRSimplify KRSimplifyTop KRDenote].
  compute.
