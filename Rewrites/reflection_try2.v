...
Inductive KRExp: Type :=
  | KRNil : KRTyp -> KRExp
  | KRVar : forall t:KRTyp, @KRrealizeType t -> @KRExp
  | KRCons : forall t:KRTyp, @KRExp -> @KRExp -> @KRExp
  | KRApp : forall t:KRTyp, @KRExp -> @KRExp -> @KRExp
...
Fixpoint KRRealizeExp (e: KRExp) :=
  match e in KRExp return option (@KRrealizeType (KRExpType e)) with
  | KRNil KRTypModuleElt => Some (@nil ModuleElt)
  | KRNil KRTypRegInitT => Some (@nil RegInitT)
  | KRNil _ => Some (@nil DefMethT)
  | KRVar t e => e
  | KRCons t a b => match KRExpType a,KRExpType b with
                    | _,_ => @nil (KRrealizeType t)
                    end
  (* @cons (@KRrealizeType t) (@KRRealizeExp a) (@KRRealizeExp b)*)
  | KRApp t a b => match KRExpType a,KRExpType b with
                   | _,_ => @nil (KRrealizeType t)
                   end
                   (*@app (@KRrealizeType t) (@KRRealizeExp (KRList t) a) (@KRRealizeExp (KRList t) b)*)
  | KRMERegister e => MERegister (@KRRealizeExp e)
  | KRMERule r => MERule (@KRRealizeExp r)
  | KRMEMeth m => MEMeth (@KRRealizeExp m)
  ...
  | KRRegisters l => @nil (ModuleElt)
      (*Registers (@KRRealizeExp l)*)
  end.
...
Fixpoint KRSimplifyTopMakeModule_regs (x:KRExp (KRList KRTypModuleElt)) :=
  ...
  | @KRApp tp a b => match tp in KRTyp return KRTyp with
                     | KRTypRegInitT => @KRApp KRTypModuleElt (KRMakeModule_regs a) (KRMakeModule_regs b)
  ...
  | q => @KRMakeModule_regs (@KRNil KRTypModuleElt)
  end.
