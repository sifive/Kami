...
Definition KRElem_decb (a:KRElem) (b:KRElem) :=
  match a,b with
...
Definition beq_KRElem' (a b: KRElem): {a = b} + {a <> b}.
Admitted.

Theorem beq_KRElem_eq: forall a b, beq_KRElem a b = true <-> a=b.
Admitted.
...
Definition beq_KRType' (a b: KRType): {a = b} + {a <> b}.
Admitted.

Theorem beq_KRType_eq: forall a b, beq_KRType a b = true <-> a=b.
Admitted.
...
Fixpoint KRExprDenote (e:KRExpr) : KRExprOption :=
  ...
  | KRCons tp f r => match KRExprDenote f, KRExprDenote r with
                     | KROption_Some tp1 f', KROption_Some (KRTypeList tp2) r' =>
                       match beq_KRType' tp1 tp2 with
                       | left isEq => KROption_Some (KRTypeList tp2) (cons match isEq in _ = Y return KRTypeDenote Y with
                                                                           | eq_refl => f'
                                                                           end r')
