...
Lemma boundProofZ (sz : nat) (w : Z):
  (w mod Z.of_nat (2^sz))%Z = w -> (w < Z.of_nat (2^sz))%Z.
Proof.
  intros sth0.
  assert (forall sz', 0 < Z.of_nat (2 ^ sz'))%Z. {
    induction sz'.
    simpl. lia.
    simpl. 
    lia. }
...
Lemma wor_wzero : forall sz w, wor _ (of_nat sz 0) w = w.
Proof.
  intros.
  arithmetizeWord.
  rewrite <- H0.
  bitblast.
  rewrite Nat.mod_0_l by discharge_pow_nonzero.
  rewrite Nat.testbit_0_l; simpl.
  rewrite H0.
  auto.
Qed.
...
Lemma wzero_wplus: forall sz w, wadd _ (of_nat sz 0) w = w.
Proof.
  intros.
  arithmetizeWord.
  simpl in *.
...
Proof.
  ...
  rewrite <- H0.
  repeat f_equal.
  rewrite Nat.lor_diag.
  auto.
Qed.

Lemma foo : forall (n x : nat) (w1 w2 : word n),
    wordVal _ (@truncMsb x _ w1) < wordVal _ (@truncMsb x _ w2) -> wordVal _ w1 < wordVal _ w2.
Proof.
  intros.
  arithmetizeWord.
  admit.
