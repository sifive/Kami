...
Section Word.
  Section fixedWidth.
    ...
    Local Notation wrap_value n := (Z.modulo n (pow2 (Z.of_nat width))).
    ...
    Definition wmax := ZToWord ((2 ^ (Z.of_nat width))%Z - 1).
    ...
    Definition wneg x := ZToWord (Z.sub (pow2 (Z.of_nat width)) (wordVal x)).
    ...
    Definition wslu x y := ZToWord (Z.mul (wordVal x) (pow2 (wordVal y))).

    Definition wsru x y := ZToWord (Z.div (wordVal x) (pow2 (wordVal y))).
  ...
  Definition truncMsb {msb outSz} (w : @word outSz) : @word msb :=
    ZToWord msb (Z.div (wordVal outSz w) (pow2 (Z.of_nat (Nat.sub outSz msb)))).


  Definition wconcat {msb lsb outSz} (w1 : @word msb) (w2 : @word lsb) :  @word outSz :=
    ZToWord outSz (Z.add (Z.mul (wordVal msb w1) (pow2 (Z.of_nat lsb))) (wordVal lsb w2)).
  ...
  Definition wnon_neg {sz} (w : @word sz) : bool :=
    (Z.ltb (wordVal _ w) (pow2 (Z.of_nat (sz - 1)))).
  
  Definition twosComplement {sz} (w: @word sz) : @word sz :=
    ZToWord _ (Z.sub (pow2 (Z.of_nat sz)) (wordVal _ w)).

  Definition wordToSignedZ {sz} (w: @word sz) : Z :=
    if Z.ltb (wordVal _ w) (pow2 (Z.of_nat (sz - 1)))
    then wordVal _ w
  ...
  Definition wsra {sz1 sz2: nat} (w1: @word sz1) (w2 : @word sz2) : @word sz1 :=
    @signedZToWord sz1 (Z.div (if wnon_neg w1
                               then wordVal _ w1
                               else wordToSignedZ w1) (pow2 (wordVal _ w2))).
  ...
  Definition wsplitl (sz1 sz2 : nat) (w : @word (sz1 + sz2)) : @word sz1 :=
    ZToWord _ (wordVal _ w / (pow2 (Z.of_nat sz2)))%Z.

  Definition wsplitr (sz1 sz2 : nat) (w : @word (sz1 + sz2)) : @word sz2 :=
    ZToWord _ ((wordVal _ w) mod (pow2 (Z.of_nat sz2)))%Z.
