...
Inductive CA_simple : Kind -> Type :=
  ...
  | CompAsyncRead_simple (idxNum num : nat) (dataArray : string) (idx : Bit (Nat.log2_up idxNum) @# ty) (k : Kind) (readMap : RME_simple) lret (cont : fullType ty (SyntaxKind (Array num k)) -> CA_simple lret) : CA_simple lret
...
Fixpoint CA_simple_of_CA{k}(a : CompActionT ty regMapTy k) : CA_simple k :=
  ...
  | CompAsyncRead idxNum num dataArray idx pred k readMap lret cont =>
      CompAsyncRead_simple idxNum dataArray idx (RME_simple_of_RME readMap) (fun x => CA_simple_of_CA (cont x))
