...
Section Simple.
  ...
  | CompAsyncRead_simple (idxNum num : nat) (readPort dataArray : string) (idx : Bit (Nat.log2_up idxNum) @# ty) (k : Kind) (readMap : RME_simple) lret (cont : fullType ty (SyntaxKind (Array num k)) -> CA_simple lret) : CA_simple lret
  ...
  Fixpoint CA_simple_of_CA{k}(a : CompActionT ty regMapTy k) : CA_simple k :=
    ...
    | CompAsyncRead idxNum num readPort dataArray idx pred k readMap lret cont =>
      CompAsyncRead_simple idxNum readPort dataArray idx (RME_simple_of_RME readMap) (fun x => CA_simple_of_CA (cont x))
