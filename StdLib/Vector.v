...
(* Definition nth {A} := *)
(*   fix nth_fix {m} (v' : t A m) (p : Fin m) {struct v'} : A := *)
(*     match m return t A m -> A with *)
(*     | O => fun v => case0 (fun _ => A) v' *)
(*     | S m' => *)
(*       match p return t A m -> A with *)
(*       | inl _ => caseS (fun n v' => A) (fun h n t => h) *)
(*       | inr p' => fun v => (caseS (fun n v' => Fin n -> A) *)
(*                                   (fun h n t p0 => nth_fix t p0) v) p' *)
(*       end *)
(*     end v'. *)
(*     match p return t A m -> A with *)
(*     | inl _ => caseS (fun n v' => A) (fun h n t => h) *)
(*     | inr p' => fun v => (caseS (fun n v' => Fin (S n) -> A) *)
(*                                (fun h n t p0 => nth_fix t p0) v ) p' *)
(*     end v'. *)
